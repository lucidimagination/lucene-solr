...
public class TestRequestHandlerResourceLastComponent extends
        SolrConfigRestTestBase {

  //
  @Test
  public void testPutRequestHandlerWithLastComponents() throws Exception {

    String content = "{"
            + "'invariants' : { 'rows' : 10 , 'echoParams' : 'explicit'} ,"
            + "'defaults' :  { 'rows'  : 100 , 'df' : 'text' } ,"
            + "'appends' :  { 'rows' : 20 } ,"
            + "'last-components' : [ 'facet' , 'query' ] }";
    ...
    assertJQ("/config/requestHandlers/select", "/class=='solr.SearchHandler'",
            ...
            "/last-components/[0]=='facet'", "/last-components/[1]=='query'");
